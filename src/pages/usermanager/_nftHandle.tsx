...
import { Input, Pagination, Spin, Button, Modal, message, Image } from 'antd';
...
const paramsWrapper = (params: Record<string, any>) => {
  for (let key in params) {
    if (['', null, undefined].includes(params[key])) {
      params[key] = void params[key];
    } else {
      continue;
    }
  }
...
export default function NFTCheckHandle({ type, tabType }: INFTCheckHandle) {
  ...
  const init = async () => {
    ...
    const res = await request.post(
      `/api/bouadmin/main/auth/${
        type === 'item'
          ? 'getitemsbylikename'
          : type === 'brand'
          ? 'getbrandsbylikename'
          : type === 'account'
          ? 'getaccountsbylikename'
          : ''
      }`,
      { data: paramsWrapper(params) },
    );
  ...
  const onDel = async (e: typeList) => {
    ...
    confirm({
      ...
      async onOk() {
        // console.log('OK');
        const res = await request.post(`/api/bouadmin/main/auth/del${type}`, {
          data: { id: e.id },
        });
        if (res.code === 200 || res.code === 1) {
      ...
      <Spin spinning={loading} tip="Loading...">
        <div style={{ width: 500 }}>
          <Search
            ...
            //   id: 'component.input.searchPlaceholder',
            // })}
            allowClear
          ...
          <div className={styles.type_item}>
            <div className={styles.row}>
              {list.map((e: any, index) => (
                <div key={e.id} className={[styles.item, 'flex'].join(' ')}>
                  <div>
                    {e.category === 'video' ? (
                      // <VideoItem width={260} height={262} src={e.fileurl} />
                      <video
                        width={162}
                        height={162}
                        src={e.fileurl}
                      ...
                      <Image src={e.fileurl} width={260} height={262} />
                    )}
                  </div>
                  <div className={styles.content}>
                    ...
                    <p className={styles.infoText}>
                      <em>owneraddress:</em> {e.owneraddress}
                    </p>
                    <p className={styles.infoText}>
                      <em>created_at:</em> {e.created_at}
                    </p>
                    <p className={styles.infoText}>
                      <em>updated_at:</em> {e.updated_at}
                    </p>
                  </div>
                  <div className={'flex flex-center-y'}>
                    <Button onClick={() => onDel(e)}>
                      <FormattedMessage id="component.button.del" />
                    </Button>
                  </div>
                </div>
              ))}
              {list.length === 0 && (
                <div className={styles.tablenull}>
                  <FormattedMessage id="component.table.null"></FormattedMessage>{' '}
                </div>
              )}
            </div>
          </div>
        ...
        {type === 'brand' && (
          <div className={styles.type_item}>
            <div className={styles.row}>
              {list.map((e, index) => (
                <div key={index} className={[styles.item, 'flex'].join(' ')}>
                  <div>
                    <Image src={e.imgurl} width={260} height={262} />
                  </div>
                  <div className={styles.content}>
              ...
              {list.map((e, index) => (
                <div key={index} className={[styles.item, 'flex'].join(' ')}>
                  ...
                  <div className={styles.content}>
                    ...
                    <p className={styles.infoText}>
                      <em>accountaddress:</em> {e.accountaddress}
                    </p>
                    <p className={styles.infoText}>
                      <em>created_at:</em> {e.created_at}
                    </p>
