...
const AuthorityTopView: React.FC<{ onSearch: (v: string) => void; run: () => void }> = ({ onSearch, run }) => {
  const intl = useIntl()
  ...
  return <>
    ...
    <Modal confirmLoading={formLoading} maskClosable={false} title="Add Administrator" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
      <Form initialValues={{ opRole: AuthorityRoleEnum.super }} form={form} onFinish={onFinish}>
        ...
        <Form.Item name="notename" label="Note Name" rules={[
          { required: true, message: 'note name not empty' }
        ]}>
          <Input allowClear placeholder="Input note name" />
        </Form.Item>
...
export default AuthorityTopView
  ...
  // const [imageUrl, setImageUrl] = useState('https://ap1-cfs3-media-bounce.bounce.finance/ae4e5320d9b8df1f2cf016971267b624-1625219729.gif')